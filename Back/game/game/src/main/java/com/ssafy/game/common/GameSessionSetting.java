package com.ssafy.game.common;

public class GameSessionSetting {
    public static final int MAX_GAME_WAIT_SECOND = 8;
    public static final int MAX_PICK_TOPIC_SECOND = 30;
    public static final int MAX_PREPARE_PRESENT_SECOND = 15;
    public static final int MAX_PRESENT_SECOND = 45;
    public static final int SMILE_COUNT_CHECK_INTERVAL_SECOND = 5;
    public static final int COUNT_DOWN_SECOND = 5;
}
