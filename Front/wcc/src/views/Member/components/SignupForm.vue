    ...
    <div>
        <div id="signupForm">
            <div class="emailInput">
                ...
                <SubmitButton v-if="emailOk && !authOk" class="innerButton sendCode" @click.prevent="sendCode"
                    value="코드 전송"></SubmitButton>
            ...
            <div class="pwInput">
                ...
                <p class="pwCondition" v-if="showPwPattern && pwWarning !== '사용 가능'">비밀번호는 8~16자의 영문 대소문자와 숫자, 특수문자를 사용하며,
                </p>
            ...
            <div class="ruleInput">
                <label class="container">
                    <input id="ruleCheck" class="ruleCheck" checked="unchecked" type="checkbox" v-model="agree">
        ...
        <CommonModal class="modal1" v-if="showRules">
            <h1>이 용 약 관</h1>
            <div class="ruleArea">‘웃참클럽’(이하 ‘회사’라 합니다)는 이용자의 개인정보를 중요시하며, 「정보통신망 이용촉진 및 정보보호 등에 관한 법률」, 「개인정보 보호법」을 준수하기 위하여
                노력하고 있습니다.<br>
                회사는 개인정보처리방침을 통하여 회사가 이용자로부터 제공받은 개인정보를 어떠한 용도와 방식으로 이용하고 있으며, 개인정보보호를 위해 어떠한 조치를 취하고 있는지 알려드립니다.<br>

                ## 0. 총칙<br>
                “개인정보”란 생존하는 개인에 관한 정보로서 당해 정보에 포함되어 있는 성명, 주민등록번호 등의 사항에 의하여 당해 개인을 식별할 수 있는 정보(당해 정보만으로는 특정 개인을 식별할 수 없더라도
                다른 정보와 용이하게 결합하여 식별할 수 있는 것을 포함)를 말합니다.<br>
                회사는 개인정보처리방침을 지속적으로 개선하기 위하여 개정에 필요한 절차를 정하고 있습니다. 또한 개인정보처리방침을 개정하는 경우 이에 버전번호 등을 부여하여 이용자가 개정된 사항을 쉽게 알아볼
                수 있게 합니다.<br>
                ...
                회사는 이용자의 정보를 수시로 저장하고 찾아내는 ‘쿠키(cookie)’ 등을 운용합니다. 쿠키란 회사의 웹사이트를 운영하는데 이용되는 서버가 이용자의 브라우저에 보내는 아주 작은 텍스트 파일로,
                ...
                ### (2) 쿠키 설정 거부 방법<br>
                이용자는 쿠키 설치에 대한 선택권을 가지고 있습니다. 따라서 웹브라우저에서 옵션을 설정함으로써 모든 쿠키를 허용하거나, 쿠키가 저장될 때마다 확인을 거치거나, 아니면 모든 쿠키의 저장을 거부할
                수도 있습니다. 단, 이용자께서 쿠키 설치를 거부하였을 경우 서비스 제공에 어려움이 있을 수 있습니다.<br>
                ...
                단, 관계법령의 규정에 의하여 보존할 필요가 있는 경우, 회사는 아래와 같이 관계법령에서 정한 일정한 기간 동안 회원정보를 보관합니다. 이 경우 회사는 해당 개인정보를 별도의
                데이터베이스(DB)로 옮기거나 보관장소를 달리하여 보존합니다.</div>
...
export default {
    ...
    methods: {
        signup() {
            console.log("pwCheckWarning: ", this.pwCheckWarning)
            if (!this.emailOk) {
                alert('이메일을 확인해주세요.')
            } else if (this.pwCheckWarning !== "일치" || this.pwWarning !== "사용 가능") {
                alert('비밀번호를 확인해주세요.')
            } else if (!this.agree) {
                alert('이용 약관에 동의해주세요.')
            } else {
